...
//    public ReviewCommentDTO readReviewCommentBy(ReviewCommentDTO dto);
    @Select("select review_comment_id, content, posted_date, review_id, member_id, parent_comment from review_comment where ${cn} = #{v}")
    public List<CustomerDTO> readReviewCommentBy(@Param("cn") String columnName, @Param("v") String value);
