...
public interface PostCommentDAO {
    @Insert("insert into post_comment ( content, posted_date, post_id, member_id, parent_comment) " +
            "values (#{content},#{postedDate},#{postId},#{memberId},#{parentComment})")
    public boolean createPostComment(PostCommentDTO dto);

    @Select("select post_comment_id, content, posted_date, post_id, member_id, parent_comment from post_comment")
    public List<PostCommentDTO>  readPostComment();

    @Update("update post_comment set content = #{content},posted_date=#{postedDate}, " +
            "member_id=#{memberId}, parent_comment=#{parentComment} where post_comment_id = #{postCommentId}")
    ...
    @Select("select post_comment_id, content, posted_date, post_id, member_id, parent_comment  from post_comment where  ${cn} = #{v}")
